"""Торговые корованы — можно грабить."""
import random
import math
...
class Korovan:
    """
    Корован движется по заданному маршруту между точками.
    ...
    def __init__(self, korovan_id, difficulty=1.0):
        self.id = korovan_id
        ...
        self.name = f'Корован {korovan_id}'
    ...
    def attack(self, player_damage, player_agility):
        """Игрок атакует Корован. Возвращает (loot_dict, gold, messages)."""
        from game.combat import resolve_attack
        messages = []

        if self.looted:
            return {}, 0, ['Корован уже ограблен']
        ...
        if self.guards > 0:
            ...
            if hit:
                ...
                if self.guard_hp <= 0:
                    self.guards -= 1
                    self.guard_hp = self.guard_max_hp
        ...
        gold = self.gold
        messages.append(f'Корован ограблен! Получено: {gold} золота')
    ...
    def info_text(self):
        lines = [
            ...
            f'Охрана: {self.guards} чел. | HP охранника: {self.guard_hp}/{self.guard_max_hp}',
            f'Товары:',
        ]
